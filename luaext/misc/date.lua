...
local function date_parse(str)
    ...
    local function setzn(zs, zn)
        zn = tonumber(zn); setz(((zn < 24) and (zn * 60) or (mod(zn, 100) + floor(zn / 100) * 60)) *
            (zs == '+' and -1 or 1))
    end
...
local function date_getdobj(v)
    ...
    return (o and o:normalize() or error "invalid date time value"),
        r -- if r is true then o is a reference to a date obj
end
--#end -- not DATE_OBJECT_AFX
