...
local select = select
local error = error
local format = string.format -- 替换成 fmt
local math = math
local os = os
local pairs = pairs
local ipairs = ipairs
...
function logging.new(append, startLevel)
  ...
  for i = 1, MAX_LEVELS do
    ...
    if logger[level:lower()] then
      return nil,
          "'" .. level .. "' is not a proper level name since there is already a property '" .. level:lower() .. "'"
    end
...
do
  ...
  if ok then
    ---@diagnostic disable-next-line: cast-local-type
    gettime = math.floor(skynet.time())
  end
  ...
  function logging.is_windows()
    local dir_separator = _ENV.package.config:sub(1, 1)
  ...
  function logging.date(fmt, t)
    ...
    local res, err = os.date(fmt, math.floor(t)) -- 5.3+ requires t to be an integer
    if type(res) == "table" then
      res.sec = ms or math.fmod(t, 1)
    end
    return res, err
  end
end
...
function logging.defaultLogPatterns(patt)
  if patt then
    ...
    for _, level in ipairs(LEVELS) do
      if level ~= "OFF" then
        assert(type(patt[level]) == "string", "the patterns contains a '%s' value (instead of a string) for level '%s'",
          type(patt[level]), level)
      end
    end
