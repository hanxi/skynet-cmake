...
function skynet.lockable(parent_queue, parent_lock)
    ...
    local lockable = function(f, lock, ...)
    ...
    if parent_queue then
        return function(f, lock, ...)
            return parent_queue(lockable, parent_lock, f, lock, ...)
            --[[
			return parent_queue(function(...)
				return lockable(f, lock, ...)
			end, parent_lock, ...)
			]] --
        end
    else
