...
struct MeshProcess: public dtTileCacheMeshProcess {
    ...
    virtual void process(
        ...
        for (int i = 0; i < params->polyCount; ++i) {
            ...
            if (polyAreas[i] == POLYAREA_GROUND || polyAreas[i] == POLYAREA_GRASS
...
class Filter {
public:
    Filter(): mFilter(new dtQueryFilter()) {
...
public:
    ...
    navmesh(const std::string& meshfile, int coord_mask = 0): coord_mask_(coord_mask) {
        if (meshfile.empty())
            return;
