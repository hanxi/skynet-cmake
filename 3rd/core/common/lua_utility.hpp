...
#define LREF_FREELIST 1
    ...
    {
        if (ref <= LREF_FREELIST || ref > lua_gettop(refL))
        {
            return false;
        }
        lua_pushnil(refL);
        while (lua_next(refL, LREF_FREELIST))
        {
            lua_pop(refL, 1);
    ...
    int luaref_ref(lua_State *refL, lua_State *L)
    {
        ...
        lua_xmove(L, refL, 1);
        lua_pushnil(refL);
        if (!lua_next(refL, LREF_FREELIST))
        {
    ...
    void luaref_unref(lua_State *refL, int ref)
    {
        ...
        if (ref < top)
        {
            ...
            lua_rawseti(refL, LREF_FREELIST, ref);
            lua_pushnil(refL);
            lua_replace(refL, ref);
            return;
        }
        for (--top; top > LREF_FREELIST; --top)
        {
            if (LUA_TNIL == lua_rawgeti(refL, LREF_FREELIST, top))
            {
                lua_pop(refL, 1);
                break;
            }
            lua_pop(refL, 1);
            lua_pushnil(refL);
            lua_rawseti(refL, LREF_FREELIST, top);
        }
        lua_settop(refL, top);
    }
    ...
    void luaref_set(lua_State *refL, lua_State *L, int ref)
    {
        assert(luaref_isvalid(refL, ref));
        lua_xmove(L, refL, 1);
        lua_replace(refL, ref);
